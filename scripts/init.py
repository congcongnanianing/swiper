...
# todo 注意：导包一定要写在加载django环境后面，否则会找不到模块。
...
def create_robots(n):
    ...
    for i in range(n):
        ...
        try:
            User.objects.create(
                phonenums = '%s' % random.randrange(21000000000, 21900000000),
                nickname = name,
                sex = sex,
...
def init_vip():
    ...
    for i in range(4):
        ...
        level = i
        price = i * 5.0
        vip, _ = Vip.objects.get_or_create(name=name,level=level,price=price)
        print('create vip %s' % vip.name)
